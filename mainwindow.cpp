...
//#include <QtGui>//Qt5中已被QtWidgets代替
#include <QtWidgets>
...
MainWindow::MainWindow()
{
    ...
    connect(scene, SIGNAL(itemInserted(DiagramItem *)),//插入图标
            this, SLOT(itemInserted(DiagramItem *)));
    connect(scene, SIGNAL(textInserted(QGraphicsTextItem *)),//插入文字
        this, SLOT(textInserted(QGraphicsTextItem *)));
    connect(scene, SIGNAL(itemSelected(QGraphicsItem *)),//选择图标
        this, SLOT(itemSelected(QGraphicsItem *)));
    createToolbars();

    QHBoxLayout *layout = new QHBoxLayout;
    layout->addWidget(toolBox);
    view = new QGraphicsView(scene);//画图区域
...
//! [23]
void MainWindow::createActions()//QAction将工具栏的动作与绘图关联
{
    toFrontAction = new QAction(QIcon(":/images/bringtofront.png"),//toFrontAction和sendBackAction一起处理绘图的叠加问题，置于上层或置于底层
                                tr("Bring to &Front"), this);
    toFrontAction->setShortcut(tr("Ctrl+F"));
    toFrontAction->setStatusTip(tr("Bring item to front"));//鼠标移动到该位置时提示
...
//    fontSizeCombo = new QComboBox;
    ...
    QIntValidator *validator = new QIntValidator(2, 64, this);//确保范围在2～64之间
